...
export default function ConfirmStep({ previewDataUrl, onAddToCart, onBack }: ConfirmStepProps) {
  ...
  return (
    <section aria-labelledby="confirm-heading" className="mx-auto max-w-6xl w-full overflow-hidden">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <VoiceAndQr
          ...
          qrToken={qrToken}
          onVoiceChange={(url, token) => {
            setVoiceDataUrl(url);
            if (url && token) setQrToken(token);
      ...
      <div className="mt-6 flex flex-col gap-2 sm:flex-row">
        ...
        <button
          type="button"
          onClick={onAddToCart}
          className="flex-1 rounded-full bg-[#B1997E] px-5 py-2.5 text-sm font-semibold uppercase tracking-wider text-white shadow-sm transition hover:bg-[#8a7460]"
        >
          Xác nhận & đặt hàng →
        </button>
      </div>
...
function CardContent({
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-stone-200 bg-white p-5 shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          placeholder="VD: Hội đồng chấm thi"
          maxLength={64}
          className="w-full min-w-0 rounded-md border border-stone-300 bg-white px-3 py-2 text-sm text-stone-800 placeholder-stone-400 focus-visible:border-[#B1997E] focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-[#B1997E]/40"
        />
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          placeholder="VD: Gửi đến Hội đồng chấm thi, chúc thầy/ cô có buổi chấm thi thật rực rỡ ạ!"
          rows={4}
          maxLength={400}
          className="resize-none rounded-md border border-stone-300 bg-white px-3 py-2 text-sm text-stone-800 placeholder-stone-400 focus-visible:border-[#B1997E] focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-[#B1997E]/40"
        />
      </div>
...
function VoiceAndQr({
  ...
  qrToken: string | null;
  onVoiceChange: (url: string | null, token?: string) => void;
}) {
  return (
  ...
  const startRecord = async () => {
    ...
    try {
      ...
      mr.onstop = () => {
        ...
        formData.append("audio", blob, `${token}.webm`);
